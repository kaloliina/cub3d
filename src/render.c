...
void	render_map(t_game *game)
{
	...
	double	pos_x = game->player.x;	//player position x
	double	pos_y = game->player.y;	//player position y
	...
	while (x < MAX_SCREEN_WIDTH)
	{
		...
		if (raydir_x < 0)	//if ray is moving left
		{
		...
		if (raydir_y < 0)	//if ray is moving up
		{
			step_y = -1;
			sidedist_y = (pos_y - map_y) * deltadist_y;
		}
		else	//if ray is moving down
		{
			step_y = 1;	//we increase y
